...
export function DynamicContent({locale}:{locale:string}){
  ...
  // Seeded CMS logos mirror the reference frontend. Only render genuinely new
  // logos here, otherwise the original partner band would appear twice.
  const referenceLogoNames=new Set(["division 31 construction","33 north homes & construction","audubon place properties","advanced renovations","better homes and gardens real estate","big","contractors corner of atlanta","am consulting & hauling","gfs","southeast restoration","ben hill renovations","neil engineering inc.","mainstreet renewal","keller williams georgia communities","exp realty","coldwell banker realty","delta carpet & decor","y studio"]);
  const logos=parsed.filter(x=>x.item.type==="logo"&&route==="/"&&!referenceLogoNames.has(x.item.title.trim().toLowerCase()));
