...
export function EditableTeam({locale,fallback}:{locale:string;fallback:Member[]}){
 ...
 return <div className="mt-16 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">{members.map((member,index)=>{const name=`${member.firstName} ${member.lastInitial}${member.lastInitial?".":""}`;return <article key={`${name}-${index}`} className="group flex h-full flex-col overflow-hidden rounded-2xl border border-white/10 bg-white/[0.04] transition hover:-translate-y-1 hover:border-gold-500/45"><div className="relative aspect-square overflow-hidden bg-navy-900"><Image src={asset(member.photo)} alt={name} fill sizes="(min-width:1024px) 28vw,(min-width:640px) 45vw,100vw" className="object-cover object-center transition-transform duration-500 group-hover:scale-[1.03]"/></div><div className="flex flex-1 flex-col p-6"><div className="flex flex-wrap items-baseline gap-2"><h3 className="text-xl text-white">{name}</h3>{member.credentials&&<span className="rounded-full border border-gold-500/35 bg-gold-500/10 px-2 py-0.5 text-[.65rem] font-bold uppercase text-gold-400">{member.credentials}</span>}</div><p className="mt-1.5 text-sm font-medium text-gold-400">{member.role}</p><p className="mt-3 text-sm leading-relaxed text-navy-100">{member.bio}</p></div></article>})}</div>
}
