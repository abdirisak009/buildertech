...
export function EditableTeam({locale,fallback}:{locale:string;fallback:Member[]}){
 ...
 // Team membership is language-independent: fetch every team row (all locales)
 // and keep one card per person (by key), preferring the current language for
 // its localized bio. This way the public page always shows the full team the
 // dashboard holds, even if a member only exists under one locale.
 useEffect(()=>{fetch(`${API_URL}/content?types=team`).then(r=>r.ok?r.json():Promise.reject()).then(x=>{const rows=x.data as Item[];const byKey=new Map<string,Item>();for(const r of rows){const cur=byKey.get(r.key);if(!cur||(r.locale===locale&&cur.locale!==locale))byKey.set(r.key,r)}const list=Array.from(byKey.values()).sort((a,b)=>a.key.localeCompare(b.key));const parsed=list.map(item=>{try{const d=JSON.parse(item.data);return {firstName:d.firstName||item.title,lastInitial:d.lastInitial||"",credentials:d.credentials||"",role:d.role||"",photo:d.photo||"",bio:d.bio||""}}catch{return null}}).filter(Boolean) as Member[];if(parsed.length)setMembers(parsed)}).catch(()=>undefined)},[locale]);
