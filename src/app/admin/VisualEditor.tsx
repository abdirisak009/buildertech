...
type Selection = { path:string; key:string; kind:"text"|"image"|"background"|"video"|"section"; value:string; alt:string; overrideId:string;fontSize?:number;width?:number;speed?:number;animated?:boolean };
...
export default function VisualEditor({token,flash,focusMode,onFocusMode}:{token:string;flash:(message:string)=>void;focusMode:boolean;onFocusMode:(value:boolean)=>void}) {
  ...
  const [fontSize,setFontSize]=useState(16);const [mediaWidth,setMediaWidth]=useState(100);const [animationSpeed,setAnimationSpeed]=useState(1);
  ...
  useEffect(()=>{
    const receive=(event:MessageEvent)=>{
      ...
      if(event.data?.type==="cms:select") {const next=event.data as Selection;setSelection(next);setValue(next.value);setAlt(next.alt||"");setFontSize(next.fontSize||16);setMediaWidth(next.width||100);setAnimationSpeed(next.speed||1)}
    };
  ...
  async function save(){if(!selection)return;setSaving(true);try{if(selection.key.startsWith("teamfield:")){const parts=selection.key.split(":");const id=parts[1];const fieldName=parts[2];const rows=await apiRequest<{id:string;locale:string;key:string;title:string;type:string;status:string;data:string}[]>("/content?types=team",{},token);const row=rows.find(r=>r.id===id);if(!row)throw new Error("Team member not found");let data:Record<string,unknown>={};try{data=JSON.parse(row.data)}catch{}let title=row.title;if(fieldName==="name"){const clean=value.trim();const m=clean.match(/^(.*?)\s+([A-Za-z])\.?$/);if(m){data.firstName=m[1].trim();data.lastInitial=m[2].toUpperCase();}else{data.firstName=clean.replace(/\.$/,"");data.lastInitial="";}title=String(data.firstName);}else{data[fieldName]=value;}await apiRequest(`/admin/contents/${id}`,{method:"PUT",body:JSON.stringify({locale:row.locale,key:row.key,title,type:row.type,status:row.status,data:JSON.stringify(data)})},token);iframe.current?.contentWindow?.location.reload();setSelection(null);flash("Team member updated");return}const metadata=JSON.stringify({alt,fontSize:selection.kind==="text"?fontSize:undefined,width:selection.kind!=="text"?mediaWidth:undefined,speed:selection.animated?animationSpeed:undefined});const item=await apiRequest<SavedOverride>("/admin/overrides",{method:"PUT",body:JSON.stringify({path:selection.path,key:selection.key,kind:selection.kind,value,alt:metadata})},token);iframe.current?.contentWindow?.postMessage({type:"cms:apply",item},window.location.origin);setSelection({...selection,overrideId:item.id,value,alt});flash("Page content saved and published")}catch(error){flash(error instanceof Error?error.message:"Save failed")}finally{setSaving(false)}}
  ...
  return <div className={`${focusMode?"h-screen":"-m-5 h-[calc(100vh-5rem)] sm:-m-8"} flex flex-col bg-slate-100`}>
    <div className="flex flex-wrap items-center gap-3 border-b border-slate-200 bg-white px-4 py-3">
      ...
      {!!sections.length&&<button onClick={()=>setShowMobileSections(true)} className="flex h-10 items-center gap-2 rounded-xl border border-slate-200 bg-white px-3 text-xs font-bold text-slate-700 hover:border-orange-300 hover:bg-orange-50"><Layers className="size-4 text-orange-600"/><span className="hidden sm:inline">Page visibility</span><span className="sm:hidden">Sections</span><span className="rounded-full bg-orange-100 px-1.5 py-0.5 text-[10px] text-orange-700">{sections.filter(x=>!x.hidden).length}/{sections.length}</span></button>}
    ...
    <div className="flex min-h-0 flex-1 gap-4 p-4">
      ...
      <aside className="hidden w-[360px] shrink-0 flex-col overflow-y-auto rounded-2xl border border-slate-200 bg-white shadow-sm lg:flex">
        ...
        <div><div className="border-b border-slate-100 p-5"><p className="text-xs font-bold uppercase tracking-[.18em] text-orange-600">{selection.kind} block</p><h3 className="mt-1 font-bold">Edit selected content</h3><p className="mt-1 truncate text-xs text-slate-400">{selection.path} · {selection.key}</p></div><div className="space-y-5 p-5">
          {selection.kind==="text"?<><label className="block text-sm font-semibold text-slate-700">Text<textarea rows={8} value={value} onChange={e=>setValue(e.target.value)} className="mt-2 w-full rounded-xl border border-slate-200 p-3 text-sm leading-6 outline-none focus:border-orange-400 focus:ring-4 focus:ring-orange-100"/></label>{selection.animated&&<p className="-mt-1 text-xs leading-5 text-slate-500">Add several phrases — one per line — and they’ll rotate (type → delete → next). A single line just types once.</p>}{selection.key.startsWith("teamfield:")?<p className="-mt-1 text-xs leading-5 text-slate-500">Editing a team member — the text saves straight to their profile. Photo, size and order are managed in Content Studio.</p>:<label className="block text-sm font-semibold text-slate-700">Text size <span className="float-right text-orange-600">{fontSize}px</span><input type="range" min="10" max="96" value={fontSize} onChange={e=>setFontSize(Number(e.target.value))} className="mt-3 w-full accent-orange-500"/></label>}{selection.animated&&<label className="block text-sm font-semibold text-slate-700">Animation speed <span className="float-right text-orange-600">{animationSpeed.toFixed(1)}×</span><input type="range" min="0.3" max="3" step="0.1" value={animationSpeed} onChange={e=>setAnimationSpeed(Number(e.target.value))} className="mt-3 w-full accent-orange-500"/></label>}</>:<>
            ...
            <label className="block text-sm font-semibold text-slate-700">Displayed size <span className="float-right text-orange-600">{mediaWidth}%</span><input type="range" min="20" max="160" value={mediaWidth} onChange={e=>setMediaWidth(Number(e.target.value))} className="mt-3 w-full accent-orange-500"/></label>
    ...
    {showMobileSections&&<div className="fixed inset-0 z-[70] bg-[#071027]/60 p-0 backdrop-blur-sm sm:p-6" onMouseDown={event=>{if(event.target===event.currentTarget)setShowMobileSections(false)}}><section className="absolute inset-x-0 bottom-0 max-h-[82vh] overflow-y-auto rounded-t-[2rem] bg-white p-5 shadow-2xl sm:relative sm:inset-auto sm:mx-auto sm:mt-[6vh] sm:max-h-[78vh] sm:max-w-2xl sm:rounded-[2rem] sm:p-7"><div className="sticky top-0 z-10 -mx-5 -mt-5 flex items-center border-b border-slate-100 bg-white px-5 py-4 sm:-mx-7 sm:-mt-7 sm:px-7 sm:py-5"><div><p className="text-xs font-bold uppercase tracking-[.18em] text-orange-600">Page visibility · {locale.toUpperCase()}</p><h3 className="mt-1 text-lg font-bold">Show or hide sections on this page</h3><p className="mt-1 text-xs text-slate-400">{path}</p></div><button onClick={()=>setShowMobileSections(false)} className="ml-auto rounded-xl bg-slate-100 p-2.5 hover:bg-slate-200" aria-label="Close page visibility"><X className="size-5"/></button></div><div className="mt-5 rounded-2xl border border-blue-100 bg-blue-50 p-4 text-sm leading-6 text-blue-800"><strong>Page-specific controls.</strong> These switches affect only this page and language. Text and images are still edited by clicking them directly in the preview.</div><div className="mt-4 flex items-center justify-between"><p className="text-sm font-bold text-slate-700">Sections</p><span className="text-xs font-semibold text-slate-400">{sections.filter(x=>!x.hidden).length}/{sections.length} live</span></div><ul className="mt-3 grid gap-2 sm:grid-cols-2">{sections.map((section,index)=><li key={section.key}><button onClick={()=>toggleSection(section)} disabled={busySection===section.key} className={`flex w-full items-center gap-3 rounded-2xl border p-4 text-left transition disabled:opacity-60 ${section.hidden?"border-slate-200 bg-slate-50 hover:bg-slate-100":"border-orange-200 bg-orange-50/50 hover:bg-orange-50"}`}><span className={`grid size-10 shrink-0 place-items-center rounded-xl ${section.hidden?"bg-slate-200 text-slate-500":"bg-orange-100 text-orange-600"}`}>{busySection===section.key?<Loader2 className="size-5 animate-spin"/>:section.hidden?<EyeOff className="size-5"/>:<Eye className="size-5"/>}</span><span className="min-w-0 flex-1"><span className={`block truncate text-sm font-bold ${section.hidden?"text-slate-500 line-through":"text-slate-800"}`}>{section.label||`Section ${index+1}`}</span><span className="mt-0.5 block text-xs text-slate-400">Click to {section.hidden?"publish":"hide"}</span></span><span className={`rounded-full px-2.5 py-1 text-[10px] font-bold uppercase ${section.hidden?"bg-slate-200 text-slate-500":"bg-emerald-100 text-emerald-700"}`}>{section.hidden?"Hidden":"Live"}</span></button></li>)}</ul></section></div>}
  </div>
}
